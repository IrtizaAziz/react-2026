...
def _value(value):
    return "UNKNOWN" if value is None else (f"{value:.6f}" if isinstance(value, (float, int)) else str(value))
...
def render_experiment_card(root, record, report):
    ...
    lines = [f"# {record.get('experiment_id', report.get('experiment_id', 'UNKNOWN'))} experiment card", "",
             ...
             "## Metrics", "", "| Metric | Score | Δ parent | Δ incumbent |", "|---|---:|---:|---:|"]
    ...
    lines += ["", "## Decision", "",
              ...
              f"- Gate evidence: {', '.join(f'{key}={value}' for key, value in gates.items()) or 'UNKNOWN'}",
...
def rebuild_experiment_index(root):
    ...
    for row in rows:
        ...
        table.append(f"| {rid} | {row.get('parent_experiment') or report.get('parent') or 'UNKNOWN'} | {change[:160]} | {row.get('model') or record.get('model') or 'UNKNOWN'} | {', '.join(spec.get('added_features', [])) or 'UNKNOWN'} | {_value(metrics.get('F1'))} | {_value(metrics.get('F2'))} | {_value(metrics.get('F2_late_corrected'))} | {_value(metrics.get('July_1_15'))} | {report.get('classification') or report.get('decision') or 'UNKNOWN'} | {report.get('submission_gate_pass', 'UNKNOWN')} | {row.get('public_lb') or 'UNKNOWN'} |")
        prov = record.get("feature_experiment_provenance", {})
        provenance.append(f"| {rid} | {spec.get('feature_module', 'UNKNOWN')} | {prov.get('module_source_sha256', 'UNKNOWN')} | {', '.join(spec.get('added_features', [])) or 'UNKNOWN'} | {len(record.get('config', {}).get('features', [])) or 'UNKNOWN'} | {report.get('certification_reused', 'UNKNOWN')} | {prov.get('parent_matrix_cache', {}).get('matrix_sha256', 'UNKNOWN') if isinstance(prov.get('parent_matrix_cache'), dict) else 'UNKNOWN'} | {prov.get('parent_matrix_cache', {}).get('used', 'UNKNOWN') if isinstance(prov.get('parent_matrix_cache'), dict) else 'UNKNOWN'} |")

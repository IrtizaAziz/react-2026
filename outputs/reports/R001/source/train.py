...
from .data import REACT2026_STATIC_FEATURES, load_training
...
def assert_static_contract(config, frame, pairs):
    ...
    if set(config.features) != set(REACT2026_STATIC_FEATURES) or forbidden & set(config.features):
    ...
    return {"feature_profile": config.feature_profile, "features": list(config.features),
            "prohibited_features_absent": True, "historical_features_absent": True,
...
def train_experiment(config, experiment, hypothesis, change, *, root=ROOT, parent=None):
    ...
    try:
        ...
        fold_runtime_seconds, fold_prediction_paths, feature_importances = [None] * len(pairs), {}, {}
        ...
        for fold in execution_order:
            ...
            feature_importances[f"F{fold + 1}"] = _feature_importance(pipeline)
        ...
        record.update(status="completed", fold_scores=scores, cv_mean=float(np.mean(scores)),
                      ...
                      feature_importances=feature_importances, diagnostics=diagnostics,

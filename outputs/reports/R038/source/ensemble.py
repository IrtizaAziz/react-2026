...
def _oof_labels_and_times(root, config, oof_frame, fingerprint):
    """Read only labels/timestamps needed to score a saved OOF blend.

    This deliberately does not materialize the feature profile: an OOF blend
    must never regenerate member features merely to compute diagnostics.
    """
    path = root / config.train_file
    if fingerprint.get("sha256") != sha256(path):
        raise ValueError("Training labels no longer match saved OOF data")
    columns = [*config.id_columns, config.target, config.time_column]
    raw = pd.read_csv(path, usecols=columns, dtype={column: "string" for column in config.id_columns})
    if raw[config.id_columns].duplicated().any():
        raise ValueError("Training data contains duplicate OOF identity IDs")
    if len(raw) != len(oof_frame) or not np.array_equal(oof_frame["__row__"].to_numpy(), np.arange(len(raw))):
        raise ValueError("OOF row identities do not exactly cover training rows")
    for column in config.id_columns:
        if not np.array_equal(oof_frame[column].astype("string").to_numpy(), raw[column].astype("string").to_numpy()):
            raise ValueError(f"OOF transaction identity mismatch for {column}")
    if raw[config.target].isna().any():
        raise ValueError("Training labels contain missing values")
    raw[config.time_column] = pd.to_datetime(raw[config.time_column], errors="raise")
    return raw


def _diagnostics(labels, assignment, blended, config):
    diagnostics = {}
    for window in config.diagnostic_windows:
        mask = ((assignment == window["fold"]) & (labels[config.time_column] >= pd.Timestamp(window["start"]))
                & (labels[config.time_column] < pd.Timestamp(window["end"])))
        diagnostics[window["name"]] = {"fold": window["fold"], "rows": int(mask.sum()),
                                        "positives": int(labels.loc[mask, config.target].sum()),
                                        "average_precision": float(score(labels.loc[mask, config.target], blended[mask], config))}
    return diagnostics
...
def blend_experiments(root, experiment, members, weights, hypothesis, change, *, method="weighted", predict_test=False, train_file=None):
    ...
    frame = _oof_labels_and_times(root, config, oof_frame, oof_meta["data_fingerprint"])
    fingerprint = oof_meta["data_fingerprint"]
    ...
    try:
        ...
        if not np.isfinite(blended).all() or (blended < 0).any() or (blended > 1).any() or not np.allclose(blended.sum(axis=1), 1, atol=1e-6, rtol=0):
            raise ValueError("Blended OOF is not finite valid probability output")
        ...
        record.update(ensemble=config.model_params, train_fingerprint=fingerprint, split_signature=oof_meta["split_signature"],
                      ...
                      diagnostics=_diagnostics(frame, assignment, blended, config),
                      aligned_member_identity={"id_columns": config.id_columns, "row_count": int(len(oof_frame)),
                                               "identity_hash": oof_meta["identity_hash"], "exact_id_alignment": True,
                                               "no_duplicate_ids": True, "no_missing_predictions": bool(np.isfinite(blended[assignment >= 0]).all())})
        if test_items:

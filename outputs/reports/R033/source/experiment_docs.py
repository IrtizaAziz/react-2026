...
def rebuild_experiment_index(root):
    ...
    for row in rows:
        ...
        table.append(f"| {rid} | {row.get('parent_experiment') or report.get('parent') or record.get('parent_experiment') or 'UNKNOWN'} | {change[:160]} | {row.get('model') or record.get('model') or 'UNKNOWN'} | {', '.join(spec.get('added_features', [])) or 'UNKNOWN'} | {_value(metrics.get('F1'))} | {_value(metrics.get('F2'))} | {_value(metrics.get('F2_late_corrected'))} | {_value(metrics.get('July_1_15'))} | {report.get('classification') or report.get('decision') or record.get('status') or 'UNKNOWN'} | {_value(report.get('submission_gate_pass', 'UNKNOWN'))} | {row.get('public_lb') or 'UNKNOWN'} |")

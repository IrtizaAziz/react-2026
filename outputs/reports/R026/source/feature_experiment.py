...
class FeatureSpec:
    ...
    def load(cls, path: Path) -> "FeatureSpec":
        ...
        allowed = {"experiment_id", "parent", "feature_module", "added_features", "hypothesis", "incumbent", "predict_test"}
        ...
        spec = cls(**{**payload, "added_features": tuple(payload["added_features"])})
        spec.validate()
        return spec
    ...
    def normalized(self):
        return {**asdict(self), "added_features": list(self.added_features), "policy_version": POLICY["version"]}
...
def _certify(root, module, available, module_hashes, *, force=False):
    ...
    if not isinstance(cases, dict) or not set(REQUIRED_CASES) <= set(cases):
        raise ValueError("Feature certification is missing required causal cases")
    results = {}
    for name, check in cases.items():
        if not callable(check): raise ValueError(f"Certification case {name} is not callable")
...
def _frame_manifest(root, parent, parent_config, parent_record, frame):
    ...
    return {"schema_version": 2, "parent": parent, "parent_config_sha256": sha256(config_path),
...
def _build_candidate(root, spec, parent_config, parent_record, module):
    ...
    if not isinstance(values, pd.DataFrame) or list(values.columns) != list(spec.added_features):
        raise ValueError("Feature module must return exactly requested columns in requested order")
    if len(values) != len(parent_frame) or not values.index.equals(raw.index):
...
def execute_spec(root, spec_path, authorize, *, recertify=False):
    ...
    provenance = {"normalized_spec": spec.normalized(), "normalized_spec_sha256": payload["spec_sha256"], "module_source_sha256": hashes,

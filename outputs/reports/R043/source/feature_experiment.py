...
class FeatureSpec:
    ...
    def load(cls, path: Path) -> "FeatureSpec":
        ...
        allowed = {"experiment_id", "parent", "feature_module", "added_features", "hypothesis", "incumbent", "predict_test", "reference_runs", "prepared_parent", "model_override"}
    ...
    def validate(self):
        ...
        if not all(isinstance(value, str) and value for value in (self.parent, self.incumbent)):
            raise ValueError("parent and incumbent must be nonempty strings")
        ...
        comparison = self.prepared_parent is not None or self.model_override is not None
        if comparison:
            if self.prepared_parent != self.parent or self.added_features or self.feature_module or not isinstance(self.model_override, dict):
                raise ValueError("Model comparison requires prepared_parent equal to parent, no feature module/delta, and a model override")
            allowed_override = {"model", "model_params", "categorical_features"}
            if set(self.model_override) - allowed_override or "model" not in self.model_override or "model_params" not in self.model_override:
                raise ValueError("Model comparison override may contain only model, model_params, and categorical_features")
        elif not self.feature_module or not self.added_features or any(not isinstance(item, str) or not item for item in self.added_features) or len(set(self.added_features)) != len(self.added_features):
            raise ValueError("Feature experiments require a module and nonempty ordered unique added_features")
...
def preflight_spec(root, spec_path):
    ...
    comparison = spec.prepared_parent is not None
    if comparison:
        module, available, hashes = None, (), {}
    else:
    ...
    cert = _certification_path(root, module.__name__, identity) if module else None
    return {"mode": "model_comparison" if comparison else "feature", "spec": spec.normalized(), "spec_sha256": object_hash(spec.normalized()), "parent": spec.parent,
            "incumbent": spec.incumbent, "feature_manifest": [*parent_config.features, *spec.added_features],
            "feature_delta": list(spec.added_features), "model_recipe": {"inherited_from": spec.parent if not comparison else None, "model": spec.model_override["model"] if comparison else parent_config.model,
            "model_params": spec.model_override["model_params"] if comparison else parent_config.model_params, "categorical_features": spec.model_override.get("categorical_features", parent_config.categorical_features) if comparison else parent_config.categorical_features,
            ...
            "certification_path": str(cert.relative_to(root)) if cert else None, "certification_reused": cert.exists() if cert else None, "test_inference_disabled": True}
...
def _oof_diversity(root, left_record, right_record, frame, config):
    """Read-only aligned ranking diagnostics for a model-family comparison."""
    left, lp, _ = read_predictions(Path(root) / left_record["oof_path"])
    right, rp, _ = read_predictions(Path(root) / right_record["oof_path"])
    if not left["__row__"].equals(right["__row__"]) or not left["__fold__"].equals(right["__fold__"]):
        raise ValueError("OOF comparison row/fold alignment failed")
    for column in config.id_columns:
        if not left[column].astype("string").equals(right[column].astype("string")):
            raise ValueError("OOF comparison transaction-ID alignment failed")
    a, b = np.asarray(lp)[:, 1], np.asarray(rp)[:, 1]
    times, folds = frame[config.time_column].astype("string").to_numpy(), left["__fold__"].to_numpy()
    result = {}
    for name, mask in {"F2": folds == 1, "July": (folds == 1) & (times >= "2026-07-01") & (times < "2026-07-16")}.items():
        x, y = a[mask], b[mask]
        finite = np.isfinite(x) & np.isfinite(y); x, y = x[finite], y[finite]
        order_x, order_y = np.argsort(-x, kind="mergesort"), np.argsort(-y, kind="mergesort")
        result[name] = {"rows": int(len(x)), "pearson": float(np.corrcoef(x, y)[0, 1]),
                        "spearman": float(pd.Series(x).corr(pd.Series(y), method="spearman")),
                        "top_1000_overlap": int(len(set(order_x[:1000]).intersection(order_y[:1000]))),
                        "top_5000_overlap": int(len(set(order_x[:5000]).intersection(order_y[:5000]))) }
    return result
...
def execute_spec(root, spec_path, authorize, *, recertify=False):
    ...
    comparison = spec.prepared_parent is not None
    if comparison:
        module, available, hashes, certification, reused = None, (), {}, {"not_applicable": "prepared parent model comparison"}, False
    else:
        module, available = _module(spec); hashes = _module_hashes(module)
    ...
    if comparison:
        candidate, fingerprint, cache = _load_parent_frame(root, spec.parent, parent_config, parent_record)
        if list(candidate.columns) != list(read_json(root / "outputs/feature_matrices" / f"{spec.parent}.json")["columns"]):
            raise ValueError("Prepared parent matrix column order differs from verified manifest")
    else:
    ...
    if comparison:
        config.model = spec.model_override["model"]; config.model_params = copy.deepcopy(spec.model_override["model_params"])
        config.categorical_features = list(spec.model_override.get("categorical_features", parent_config.categorical_features))
    ...
    change = (f"Reuse verified immutable {spec.parent} prepared matrix with no feature changes; override only the model/preprocessing recipe."
              if comparison else f"Append {list(spec.added_features)} from {spec.feature_module} to immutable {spec.parent}; inherit {spec.parent}'s complete model recipe.")
    ...
    diagnostics_path, diagnostics_error = (None, None) if comparison else _run_diagnostics(root, module, candidate, record, config, spec.added_features)
    diversity = _oof_diversity(root, parent_record, record, candidate, config) if comparison else None
    if comparison:
        strict = {"F2": .003, "F2_late_corrected": .002, "July_1_15": .003, "F1": -.002, "July_1_7": -.002, "July_8_15": -.002}
        competitive = {"F2": 0, "F2_late_corrected": 0, "July_1_15": 0, "F1": -.002, "July_1_7": -.002, "July_8_15": -.002}
        model_classification = "CLEAR WIN" if all(incumbent_delta.get(key, -np.inf) >= value for key, value in strict.items()) else ("COMPETITIVE-BLEND-ELIGIBLE" if all(incumbent_delta.get(key, -np.inf) >= value for key, value in competitive.items()) else "CLOSE")
    ...
    report = {"experiment_id": spec.experiment_id, "parent": spec.parent, "incumbent": spec.incumbent, "metrics": candidate_scores,
              ...
              "reference_runs": references, "diversity": diversity, "diagnostics_path": diagnostics_path, "diagnostics_error": diagnostics_error,

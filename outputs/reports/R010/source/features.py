...
def build_pipeline(config):
    ...
    if config.model == "catboost":
        ...
        numeric_columns = [column for column in config.features if column not in categorical_columns]
        ...
        preprocessing = ColumnTransformer([
            ...
            ("categorical", categorical, categorical_columns)], verbose_feature_names_out=False).set_output(transform="pandas")
        # SimpleImputer's pandas output may not preserve the original column
        # names across supported sklearn versions. ColumnTransformer keeps this
        # numeric-then-categorical order, so indices are the stable native-CatBoost
        # categorical reference for the transformed fold data.
        model.set_params(cat_features=list(range(len(numeric_columns), len(config.features))))

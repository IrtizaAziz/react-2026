...
from .data import REACT2026_CUSTOMER_HISTORY_FEATURES, REACT2026_CUSTOMER_RELATIONSHIP_FEATURES, REACT2026_DEVICE_GLOBAL_FEATURES, REACT2026_STATIC_FEATURES, REACT2026_VELOCITY_FEATURES, REACT2026_BROAD_VELOCITY_FEATURES, REACT2026_MERCHANT_ONEHOT_PROFILE, load_training
...
def assert_static_contract(config, frame, pairs):
    ...
    if config.feature_profile in {"react2026_static_customer_history_relationships_device_global", "react2026_static_customer_history_relationships_device_global_velocity", "react2026_static_customer_history_relationships_device_global_broad_velocity", REACT2026_MERCHANT_ONEHOT_PROFILE}:
...
def _feature_importance(pipeline, feature_names, *, config=None):
    ...
    if config is not None and config.model == "lightgbm":
        transformed = list(pipeline.named_steps["preprocess"].get_feature_names_out())
        ...
        for name, value in zip(transformed, values):
            if name in grouped:
                ...
                continue
            matched = [raw for raw in categorical if name.startswith(raw + "_")]
            if len(matched) != 1:
                raise ValueError(f"Unmapped LightGBM transformed feature: {name}")
            grouped[matched[0]] += float(value)
        return [{"feature": name, "importance": value} for name, value in

...
def record_failure(root, spec_path, reason, *, stage="pre_training_infrastructure"):
    """Consume the next immutable ID for an authorized failure before training."""
    import csv
    from .feature_experiment import FeatureSpec
    root = Path(root); spec_path = Path(spec_path)
    if not spec_path.exists():
        raise FileNotFoundError(f"Missing experiment spec: {spec_path}")
    spec = FeatureSpec.load(spec_path)
    if spec.experiment_id != _next_id(root):
        raise ValueError(f"Experiment ID {spec.experiment_id} is not the next unused immutable ID")
    record_path = root / "outputs/reports" / f"{spec.experiment_id}.json"
    if record_path.exists():
        raise ValueError(f"Experiment ID {spec.experiment_id} is already consumed")
    parent_path = root / "outputs/reports" / f"{spec.parent}.json"
    parent = read_json(parent_path) if parent_path.exists() else {}
    timestamp = datetime.now(timezone.utc).isoformat()
    record = {
        "experiment_id": spec.experiment_id, "timestamp": timestamp, "status": "failed",
        "parent_experiment": spec.parent, "hypothesis": spec.hypothesis,
        "change_description": f"Authorized pre-training failure: {reason}",
        "feature_module": spec.feature_module, "added_features": list(spec.added_features),
        "model": parent.get("model", "UNKNOWN"), "failure_stage": stage,
        "failure_reason": reason, "training_started": False, "oof_created": False,
        "test_inference": False, "submission_created": False,
        "feature_experiment_provenance": {"normalized_spec": spec.normalized(), "spec_path": str(spec_path.resolve())},
    }
    save_json(record_path, record, exclusive=True)
    ledger = root / "experiments/experiments.csv"; ledger.parent.mkdir(parents=True, exist_ok=True)
    existing = []
    if ledger.exists():
        with ledger.open(newline="", encoding="utf-8") as handle: existing = list(csv.DictReader(handle))
    if any(row.get("experiment_id") == spec.experiment_id for row in existing):
        raise ValueError(f"Experiment ID {spec.experiment_id} is already present in the ledger")
    fields = list(existing[0].keys()) if existing else ["experiment_id", "timestamp", "parent_experiment", "model", "hypothesis", "change_description", "status"]
    row = {field: "" for field in fields}
    row.update({"experiment_id": spec.experiment_id, "timestamp": timestamp, "parent_experiment": spec.parent,
                "model": parent.get("model", "UNKNOWN"), "hypothesis": spec.hypothesis,
                "change_description": record["change_description"], "status": "failed"})
    with ledger.open("a", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields); writer.writerow(row)
    return record
...
def _blend_preflight(root, parent, members, weights):
    ...
    from .ensemble import blend_covered_oofs
    from .predict import read_predictions
    _frame, _blended, _metadata, covered = blend_covered_oofs([read_predictions(item["path"]) for item in oof_details], weights)
    return {"mode": "blend", "parent": parent, "members": list(members), "weights": weights, "method": "weighted",
            ...
            "canonical_covered_rows": int(covered.sum()), "canonical_uncovered_rows": int((~covered).sum()),
            "expected_null_masks_match": True}
...
def main(argv=None):
    ...
    cache = sub.add_parser("cache-parent", help="materialize or verify a completed immutable training feature cache")
    cache.add_argument("--run", required=True); cache.add_argument("--verify-only", action="store_true")
    wait.add_argument("--poll-seconds", type=float, default=25); wait.add_argument("--timeout-seconds", type=float, default=1800)
    failure = sub.add_parser("record-failure"); failure.add_argument("--spec", required=True); failure.add_argument("--reason", required=True); failure.add_argument("--stage", default="pre_training_infrastructure")
    for name in ("preflight", "run", "supervise"):
        command = sub.add_parser(name); command.add_argument("mode", choices=("training", "blend"), nargs="?"); command.add_argument("--parent")
        ...
        if name in {"run", "supervise"}:
            command.add_argument("--experiment"); command.add_argument("--hypothesis"); command.add_argument("--change"); command.add_argument("--authorize"); command.add_argument("--recertify", action="store_true")
    ...
    try:
        if getattr(args, "spec", None):
            from .feature_experiment import FeatureSpec, execute_spec, preflight_spec
            if args.command == "record-failure":
                result = record_failure(args.root, args.spec, args.reason, stage=args.stage)
                from .experiment_docs import rebuild_experiment_index
                rebuild_experiment_index(args.root)
                print(json.dumps(result, indent=2)); return 0
            if args.mode or args.parent or args.config or args.tests or args.members or args.weights:
                raise ValueError("--spec cannot be combined with legacy experiment arguments")
            spec = FeatureSpec.load(args.spec)
            if args.command == "preflight":
                result = preflight_spec(args.root, args.spec)
            elif args.command == "supervise":
                if not args.authorize: raise ValueError("Feature execution requires --authorize <exact_R_ID>")
                child = ["--root", str(args.root), "run", "--spec", args.spec, "--authorize", args.authorize]
                if args.recertify: child.append("--recertify")
                return supervise(args.root, child, spec.experiment_id, args.heartbeat_seconds)
            else:
                if not args.authorize: raise ValueError("Feature execution requires --authorize <exact_R_ID>")
                result = execute_spec(args.root, args.spec, args.authorize, recertify=args.recertify)
        elif args.command == "preflight":
            if not args.mode or not args.parent: raise ValueError("Legacy preflight requires mode and --parent")
        ...
        elif args.command == "cache-parent":
            from .feature_experiment import cache_parent
            result = cache_parent(args.root, args.run, verify_only=args.verify_only)
        elif args.command == "supervise":
            if not args.mode or not args.parent or not args.experiment or not args.hypothesis or not args.change: raise ValueError("Legacy supervise requires mode, parent, experiment, hypothesis, and change")
            return supervise(args.root, _run_argv(args), args.experiment, args.heartbeat_seconds)
        else:
            if not args.mode or not args.parent or not args.experiment or not args.hypothesis or not args.change: raise ValueError("Legacy run requires mode, parent, experiment, hypothesis, and change")

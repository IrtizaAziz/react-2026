...
WEEK_NS = np.int64(604_800_000_000_000)
BROAD_FEATURES = ["customer_prior_24h_count", "customer_prior_7d_count", "customer_1h_vs_7d_rate_ratio",
                  "device_prior_24h_count", "device_prior_7d_count", "device_1h_vs_7d_rate_ratio"]
...
def _broad_empty(n):
    result = _empty(n)
    result.update({name: np.zeros(n, dtype=np.int64) for name in BROAD_FEATURES if name.endswith("_count")})
    result.update({name: np.zeros(n, dtype=float) for name in BROAD_FEATURES if name.endswith("_ratio")})
    return result


def _broad_velocity_with_state(timestamp, customer_id, device_id, state):
    """Three exact rolling windows; state is retained independently per entity/window."""
    times, customers, devices = _input_arrays(timestamp, customer_id, device_id)
    customer_state, device_state = state
    out = _broad_empty(len(times))
    windows = ((WINDOW_NS, "1h"), (DAY_NS, "24h"), (WEEK_NS, "7d"))
    starts = np.r_[0, np.flatnonzero(times[1:] != times[:-1]) + 1]
    ends = np.r_[starts[1:], len(times)]
    for start, end in zip(starts, ends):
        current = times[start]
        for keys, histories, prefix in ((customers, customer_state, "customer"), (devices, device_state, "device")):
            counts = {}
            for window, suffix in windows:
                history = histories[suffix]
                for entity in np.unique(keys[start:end]):
                    _evict(history[entity], current if window == WINDOW_NS else current + (WINDOW_NS - window))
                # _evict keeps timestamps >= (argument - 1 hour); transform the
                # argument to reuse its exact inclusive-boundary implementation.
                counts[suffix] = np.asarray([len(history[entity]) for entity in keys[start:end]], dtype=np.int64)
            out[f"{prefix}_prior_1h_count"][start:end] = counts["1h"]
            out[f"{prefix}_prior_24h_count"][start:end] = counts["24h"]
            out[f"{prefix}_prior_7d_count"][start:end] = counts["7d"]
            out[f"{prefix}_1h_vs_7d_rate_ratio"][start:end] = (counts["1h"] + 1.0) / (counts["7d"] / 168.0 + 1.0)
        for keys, histories in ((customers, customer_state), (devices, device_state)):
            for _, suffix in windows:
                for entity in keys[start:end]:
                    histories[suffix][entity].append(current)
    return pd.DataFrame(out)


def broad_velocity_oracle(timestamp, customer_id, device_id):
    """Simple all-window reference implementation for synthetic causal tests."""
    times, customers, devices = _input_arrays(timestamp, customer_id, device_id)
    out = _broad_empty(len(times))
    for i, current in enumerate(times):
        prior = times[:i] < current
        for keys, prefix in ((customers, "customer"), (devices, "device")):
            counts = {}
            for window, suffix in ((WINDOW_NS, "1h"), (DAY_NS, "24h"), (WEEK_NS, "7d")):
                counts[suffix] = int(np.sum(prior & (times[:i] >= current - window) & (keys[:i] == keys[i])))
                out[f"{prefix}_prior_{suffix}_count"][i] = counts[suffix]
            out[f"{prefix}_1h_vs_7d_rate_ratio"][i] = (counts["1h"] + 1.0) / (counts["7d"] / 168.0 + 1.0)
    return pd.DataFrame(out)


def broad_velocity_production(timestamp, customer_id, device_id):
    state = ({suffix: defaultdict(deque) for suffix in ("1h", "24h", "7d")},
             {suffix: defaultdict(deque) for suffix in ("1h", "24h", "7d")})
    return _broad_velocity_with_state(timestamp, customer_id, device_id, state)


def broad_velocity_chunked(chunks):
    state = ({suffix: defaultdict(deque) for suffix in ("1h", "24h", "7d")},
             {suffix: defaultdict(deque) for suffix in ("1h", "24h", "7d")})
    ...
    for chunk in chunks:
        ...
        if ready_end:
            ...
            outputs.append(_broad_velocity_with_state(ready["timestamp"], ready["customer_id"], ready["device_id"], state).set_axis(ready.index))
        pending = combined.iloc[ready_end:]
    if len(pending):
        outputs.append(_broad_velocity_with_state(pending["timestamp"], pending["customer_id"], pending["device_id"], state).set_axis(pending.index))
    return pd.concat(outputs).sort_index() if outputs else pd.DataFrame(columns=[*FEATURES, *BROAD_FEATURES])

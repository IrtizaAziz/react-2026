"""Two-step immutable REACT experiment runner; never exports test predictions."""
import argparse
import hashlib
import json
import re
...
def preflight(root, mode, **kwargs):
    ...
    payload["authorization_token"] = hashlib.sha256(_canonical(payload).encode()).hexdigest()[:20]
    return payload
...
def run(root, payload, hypothesis, change, token, experiment):
    root = Path(root); expected = hashlib.sha256(_canonical({key: value for key, value in payload.items() if key != "authorization_token"}).encode()).hexdigest()[:20]
    if experiment != payload["experiment_id"]: raise ValueError("Explicit experiment ID does not match this exact preflight")
    if token != payload.get("authorization_token") or token != expected: raise ValueError("Authorization token does not match this exact preflight")
...
def main(argv=None):
    ...
    for name in ("preflight", "run"):
        ...
        if name == "run": command.add_argument("--experiment", required=True); command.add_argument("--token", required=True); command.add_argument("--hypothesis", required=True); command.add_argument("--change", required=True)
    ...
    try:
        if args.command == "preflight":
            ...
            result = run(args.root, payload, args.hypothesis, args.change, args.token, args.experiment)

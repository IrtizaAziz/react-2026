...
def run(root=ROOT, experiment=EXPERIMENT):
    ...
    if not r044_frame["transaction_id"].equals(a_frame["transaction_id"]) or not np.allclose(r044_values[covered], blended[covered], atol=1e-15, rtol=0) or not np.isnan(r044_values[~covered]).all():
